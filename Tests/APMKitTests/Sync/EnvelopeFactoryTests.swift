...
struct EnvelopeFactoryTests {
    ...
    func wrapsEventsWithContext() {
        ...
        #expect(envelope.userIdSource == .host)
        #expect(envelope.app.id == "com.example.app")
        #expect(envelope.events.map(\.eventId) == events.map(\.eventId))
    }

    @Test("threads an isolated UserIdentity-backed userIdentity closure through unchanged, id and source both")
    func threadsUserIdentityThrough() throws {
        ...
        let factory = EnvelopeFactory(
            sessionManager: SessionManager(),
            userIdentity: { UserIdentity.currentUserIdentity(userDefaults: defaults) }
        )
    ...
    @Test("sdk.health is read fresh from selfHealth.snapshot() on every makeEnvelope call")
    func sdkHealthReflectsSelfHealthSnapshot() {
        let selfHealth = SelfHealthCounters()
        selfHealth.recordWritten(5)
        selfHealth.recordSent(3)
        selfHealth.recordDropped(2, reason: "queue_full")

        let factory = EnvelopeFactory(sessionManager: SessionManager(), selfHealth: selfHealth)
        let envelope = factory.makeEnvelope(events: [])

        #expect(envelope.sdk.health?.written == 5)
        #expect(envelope.sdk.health?.sent == 3)
        #expect(envelope.sdk.health?.dropped == 2)
        #expect(envelope.sdk.health?.dropReasons["queue_full"] == 2)
    }
